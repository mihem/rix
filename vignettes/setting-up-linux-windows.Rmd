...
`rstats-on-nix` cache! Alternatively (or in addition), you can use the [binary
attic cache](https://github.com/mihem/attic) by [Mischko Heming](https://mheming.com).
It has the advantage of containing all R packages available in Nixpkgs (~32,000),
with weekly updates since 2026-05-18. However, this is a private project running
at a much smaller scale than the professional Cachix servers, and it only works
for x86_64 Linux. For instructions, see
[the section below](#using-the-binary-attic-cache).
...
### Using the binary attic cache

To use the [binary attic cache](https://github.com/mihem/attic) by
[Mischko Heming](https://mheming.com), configure your Nix settings as follows.
If you are on NixOS, add the following to your `configuration.nix`:
...
For a flake-based project, you can use this in `flake.nix`:
...
On NixOS, rebuild your system with `sudo nixos-rebuild switch` (or
`home-manager switch` for Home Manager). For regular Nix installations and
flake-based projects, the settings are picked up on the next Nix operation.
